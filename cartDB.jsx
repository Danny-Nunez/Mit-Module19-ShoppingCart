...
const Products = (props) => {
  ...
  let cartList = cart.reduce((acc, item) => {
    ...
    return (
      <Card key={index}>
        <div class="wrapper">
        <div class="text">$ {item.cost} {item.name} (Qty {item.quantity})  from {item.country}{" "} </div>
          <button type="button" class="btn btn-danger" onClick={() => deleteCartItem(index)} eventKey={1 + index}>
            Remove
          </button>
        </div>
      </Card>
